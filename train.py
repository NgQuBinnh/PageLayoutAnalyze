...
DATA_LIST_PATH = './dataset/train_plad.txt'
...
def main():
    ...
    # image_val_batch, label_val_batch = tf.expand_dims(image, dim=0), tf.expand_dims(label, dim=0)
    ...
    print("Shape of raw output = " + str(raw_output.get_shape))
    raw_prediction = tf.reshape(raw_output, [-1, args.num_classes])
    print("After reshape = " + str(raw_prediction.get_shape))
    ...
    print("pred shape = " + str(pred.get_shape))

    # pred_val = tf.reshape(pred, [-1, ])
    # ground_truth = tf.reshape(label_val_batch, [-1, ])
    # weights = tf.cast(tf.less_equal(ground_truth, args.num_classes - 1),
    #                   tf.int32)  # Ignoring all labels greater than or equal to n_classes.
    # mIoU, update_op = tf.contrib.metrics.streaming_mean_iou(pred_val, ground_truth, num_classes=args.num_classes, weights=weights)
    ...
    saver = tf.train.Saver(var_list=tf.global_variables(), max_to_keep=15)
